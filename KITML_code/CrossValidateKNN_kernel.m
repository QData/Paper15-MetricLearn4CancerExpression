function [acc, pred,y] = CrossValidateKNN_kernel(y, X, tCL, k, knn_size);
...
for (i=1:k),
   ...
   %train model
   M = feval(tCL, yt, Xt); % it is KA here, so KNN need to be re-write, which take KA
   
   %evaluate model 
   XT = X(test_start:test_end, :);
   yT =  y(test_start:test_end);
   pred(test_start:test_end) = KNN_kernel(yt, Xt, M, knn_size, XT); 
end
acc = sum(pred==y)/n;
